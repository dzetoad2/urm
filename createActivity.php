...
//======================================================
    ...
    if (isset($_POST['createSubmit'])) {
        ...
        if ($afDao->errorLabel == '') { // if still no error here, then its ok to create in the db.
            $activityTypeLabel = '';
            if (!isset($_POST['fid'])) {
                throwMyExc('submitsurveyanswer(): post fid not set');
            }
            if (!isset($_POST['is_cf']))
                throwMyExc('submitsurveyanswer(): post is_cf not set');
            $fid = $_POST['fid'];
            $is_cf = $_POST['is_cf'];
            if ($activityType == "customActivity") {
...
// -------- MANIPULATE AFDAO TO YIELD OUTPUT INTO THIS FORM:
                ...
                <div>
                    <input type="submit" name="createSubmit"
                           ...
                           value="<?php
            if ($activityType == "customActivity")
                echo "Create and add to my User Created Activities";
            else if ($activityType == "activity")
                echo "Create and add to Activities";
            else {
            ...
            if (defined('DEBUG')) {
                ...
                if (isset($surveyCategoryId))
                    echo "Your survey category id is: " . $surveyCategoryId . '<br/>';
            }
            ?>
